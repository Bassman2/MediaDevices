...
public abstract class WritableUnitTest : UnitTest
{
    ...
    public void WritablePersistentUniqueIdTest()
    {
        ...
        var dir = device.GetDirectoryInfo(Path.Combine(this.workingFolder!, @"UploadTree\Aaa\Abb"));
        Assert.IsNotNull(dir, nameof(dir));
        string dirPui = dir.PersistentUniqueId;
        var dirGet = device.GetFileSystemInfoFromPersistentUniqueId(dirPui) as MediaDirectoryInfo;

        var file = device.GetFileInfo(Path.Combine(this.workingFolder!, @"UploadTree\Aaa\Abb\Acc\Ctest.txt"));
